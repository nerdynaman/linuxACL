...
struct acl* initACL(char* path){
    // read acl file and store it in a data structure
    // return the data structure
    const char* xattr_name = "acl";
    char buffer[256];
	ssize_t xattr_length = getxattr(path, xattr_name, buffer, sizeof(buffer),0,0);
    struct acl *aclList = (struct acl*)malloc(sizeof(struct acl));
    if (xattr_length == -1) {
		// std::cout << "No xattr found" << std::endl;
        return aclList;
    }
    buffer[xattr_length] = '\0';
	char* lineDup = strdup(buffer);
...
int checkAcessPerms(char* path, char* user, char* perm, char* type){
    ...
    struct acl *aclList = initACL(path); // get the acl list
    if (aclList == NULL){
